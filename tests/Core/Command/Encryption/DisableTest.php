...
class DisableTest extends TestCase {
	...
	public function dataDisable() {
		return [
			...
			['yes', true, '1', 3, 'Encryption disabled'],
			['yes', true, '', 14, 'Encryption disabled'],
			['no', false, false, 7, 'Encryption is already disabled'],
		];
	}
	 ...
	 * @param int $encryptedFilesCount
	 * @param string $expectedString
	 */
	public function testDisable($oldStatus, $isUpdating, $masterKeyEnabled, $encryptedFilesCount, $expectedString) {
		$stmt = $this->createMock(Statement::class);
		$stmt->method('fetchColumn')
			->willReturn($encryptedFilesCount);
